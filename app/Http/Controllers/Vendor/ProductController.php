...
class ProductController extends Controller
{
    ...
    public function datatables()
    {
        ...
        foreach ($datas as $data) {
            $name = $data->name;
            if (isJSON($data->name)) {
                $name = json_decode($data->name, true)[$language->id];
            }
            $data->name = $name;
            $items[] = $data;
        }
    ...
    {
        $user = Auth::user();
        $datas = $user->products()->where('product_type', 'normal')->where('is_catalog', '=', 1)->orderBy('id', 'desc')->get();
        $language = Language::where('is_default',1)->first();
        $items = [];
        foreach ($datas as $data) {
            $name = $data->name;
    ...
    public function createPhysical()
    {
        ...
        return view('vendor.product.create.physical', compact('cats', 'sign', 'languages'));
    }
    ...
    public function createDigital()
    {
        ...
        $sign = Currency::where('is_default', '=', 1)->first();
        $languages = Language::all();
        return view('vendor.product.create.digital', compact('cats', 'sign','languages'));
    }
    ...
    public function createLicense()
    {
        ...
        $sign = Currency::where('is_default', '=', 1)->first();
        $languages = Language::all();
        return view('vendor.product.create.license', compact('cats', 'sign','languages'));
    }
...
//                $input['childcategory_id'] = $request->childcategory_id;
    ...
    public function store(Request $request)
    {
        ...
        if ($prods < $package->allowed_products || $package->allowed_products == 0) {
            ...
            $input['name'] = json_encode($request->input('name'));
            $input['details'] = json_encode($request->input('details'));
            $input['policy'] = json_encode($request->input('policy'));
            $data->fill($input)->save();
    ...
    public function edit($id)
    {
        ...
        if ($data->type == 'Digital')
            return view('vendor.product.edit.digital', compact('cats', 'data', 'sign','languages'));
        elseif ($data->type == 'License')
            return view('vendor.product.edit.license', compact('cats', 'data', 'sign','languages'));
        else
            return view('vendor.product.edit.physical', compact('cats', 'data', 'sign','languages'));
    }
    ...
    /*    public function update(Request $request, $id)
        {
            ...
            if ($request->photo) {
                ...
                // Set Thumbnail
                $img = \Intervention\Image\Facades\Image::make(public_path() . '/assets/images/products/' . $data->photo)->resize(285, 285);
                $thumbnail = time() . Str::random(8) . '.jpg';
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($file = $request->file('photo')) {
            // Set Photo
            $resizedImage = Image::make(public_path() . '/assets/images/products/' . $data->photo)->resize(800, null, function ($c) {
                $c->aspectRatio();
            });
            $photo = time() . Str::random(8) . '.jpg';
            $resizedImage->save(public_path() . '/assets/images/products/' . $photo);
