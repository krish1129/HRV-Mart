...
class CatalogController extends Controller
{
    ...
    public function category(Request $request, $slug = null, $slug1 = null, $slug2 = null)
    {
        ...
        $datas = $prods->where('status', 1)->get();

        $datas = (Product::filterProducts($datas));
    ...
    public function product($slug)
    {
        ...
        $check = Noti::where('product_id',$pro->id)->where('session_id',request()->ip())->first();
        if(!$check) {
            $store = new Noti;
            $store->product_id = $pro->id;
            $store->session_id = request()->ip();
            $store->status = 1;
            $store->save();
        }
        /////////////////////////
        $this->code_image();
        $productt = Product::where('slug', '=', $slug)->with('user')->firstOrFail();
