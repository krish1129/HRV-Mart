...
class ChildCategoryController extends Controller
{
   public function __construct()
        ...
        foreach ($datas as $data) {
         ...
         return Datatables::of($datas)
                            ->addColumn('category', function(Childcategory $data) {
                                ...
                                if (isJSON($data->subcategory->category->name)) {
                                    $name = json_decode($data->subcategory->category->name, true)[$language->id];
                                }
                                $data->subcategory->category->name = $name;
                            ...
                            ->addColumn('subcategory', function(Childcategory $data) {
                                ...
                                if (isJSON($data->subcategory->name)) {
                                    $name = json_decode($data->subcategory->name, true)[$language->id];
                                }
    ...
    public function create()
    {
        ...
        foreach ($datas as $data) {
            foreach ($data->subs as $sub){
                $name = $sub->name;
                if (isJSON($sub->name)) {
                    $name = json_decode($sub->name, true)[$language->id];
                }
                $sub->name = $name;
            }
            $cats[] = $data;
        }
        return view('admin.childcategory.create',compact('cats','languages'));
    }
    ...
    public function edit($id)
    {
        ...
        return view('admin.childcategory.edit',compact('data','cats','subcats','languages'));
    }
...
//        $language = Language::where('is_default',1)->first();
//        $name = $subcat->name;
//        if (isJSON($subcat->name)) {
//            $name = json_decode($subcat->name, true)[$language->id];
//        }
//        $subcat->name = $name;
