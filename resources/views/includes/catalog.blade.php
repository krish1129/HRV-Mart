            ...
            <div class="body-area">
              <form id="catalogForm" action="{{ route('front.category', [Request::route('category'), Request::route('subcategory'), Request::route('childcategory')]) }}" method="GET">
                ...
                <ul class="filter-list">
                  ...
                  <li>
                    <div class="content">
                        <a href="{{route('front.category', $element->slug)}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="category-link"> <i class="fas fa-angle-double-right"></i>@if(isJson($element->name)){{ json_decode($element->name,true)[$langg->id]}}@else {{$element->name}} @endif</a>
                        @if(!empty($cat) && $cat->id == $element->id && !empty($cat->subs))
                            @foreach ($cat->subs as $key => $subelement)
                            <div class="sub-content open">
                              <a href="{{route('front.category', [$cat->slug, $subelement->slug])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="subcategory-link"><i class="fas fa-angle-right"></i>@if(isJson($subelement->name)){{ json_decode($subelement->name,true)[$langg->id]}}@else {{$subelement->name}} @endif</a>
                              @if(!empty($subcat) && $subcat->id == $subelement->id && !empty($subcat->childs))
                                ...
                                <div class="child-content open">
                                  <a href="{{route('front.category', [$cat->slug, $subcat->slug, $childcat->slug])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="subcategory-link"><i class="fas fa-caret-right"></i>@if(isJson($childcat->name)){{ json_decode($childcat->name,true)[$langg->id]}}@else {{$childcat->name}} @endif</a>
                                </div>
                                @endforeach
                              @endif
                            </div>
