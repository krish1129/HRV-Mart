...
@if(Carbon\Carbon::now()->format('Y-m-d') < Carbon\Carbon::parse($prod->discount_date)->format('Y-m-d'))
												...
												<a href="{{ route('front.product', $prod->slug) }}" class="item">
													...
													<div class="info">
														<h5 class="name">@if(isJson($prod->name)){{ json_decode($prod->name ,true)[$langg->id]}}@else {{$prod->name}} @endif</h5>
														<div class="stars">
																											<div class="ratings">
           ...
           $previous_price=preg_replace('/[^0-9,.]+/', '', $prod->showPreviousPrice());
                                                   ...
                                                   $percentage =number_format( ( $diff/ $previous_price)*100);
													...
													<div class="item-cart-area position-relative">
				...
				<a href="{{ route('front.product', $prod->slug) }}" class="item">
					...
					<div class="info">
						...
						<h4 class="price">{{ $prod->showPrice() }} <del><small>{{ $prod->showPreviousPrice() }}</small></del></h4>
								...
								<div class="item-cart-area position-relative">
															@if($prod->product_type == "affiliate")
																<span class="add-to-cart-btn affilate-btn" data-href="{{ route('affiliate.product', $prod->slug) }}"><i class="sagor"></i> {{ $langg->lang251 }}
																</span>
															@else
																<span class="add-to-cart add-to-cart-btn" data-href="{{ route('product.cart.add',$prod->id) }}">
																	<i class="sagor"></i> {{ $langg->lang56 }}
