                          ...
                          <div class="col-lg-7">
                              <select id="cat" required="">
                                  <option value="">{{ __('Select Category') }}</option>
                                    @foreach($cats as $cat)
                                      <option data-href="{{ route('admin-subcat-load',$cat->id) }}" value="{{ $cat->id }}">@if(isJSON($cat->name)){{json_decode( $cat->name,true)[$alang->id]}}@else {{$cat->name}} @endif</option>
                                    @endforeach
                                </select>
                          </div>
                        </div>
                              ...
                              <div class="row">
                                  ...
                                  <div class="col-lg-7">
                                      <input type="text" class="input-field" name="name[{{$language->id}}]"
                                             placeholder="{{ __('Enter Name (In '.$language->language.')') }}"
                                             required="" value="">
                                  </div>
                              </div>
                          @endforeach
