                                ...
                                @foreach($productss as $productt)
                                    @foreach($productt as $prod)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                @if(isJSON($prod->product->name)){{json_decode($prod->product->name,true)[$alang->id]}}@else {{$prod->product->name}} @endif
{{--                                                {{mb_strlen($prod->product->name,'utf-8') > 60 ? mb_substr($prod->product->name,0,60,'utf-8').'...' : $prod->product->name}}--}}
                                            </td>
                                            <td>
                                                @if(isJSON($prod->product->category->name)){{json_decode($prod->product->category->name,true)[$alang->id]}}@else {{$prod->product->category->name}} @endif
