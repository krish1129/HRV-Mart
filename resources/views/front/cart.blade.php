            ...
            <table class="table">
              @include('includes.form-success')
                <thead>
                    <tr>
                      <th>{{ $langg->lang122 }}</th>
                      <th class="sub__total" width="20%">{{ $langg->lang539 }}</th>
                      <th>{{ $langg->lang125 }}</th>
                      <th>{{ $langg->lang126 }}</th>
                  ...
                  <tbody>
                    ...
                    <tr class="cremove{{ $product['item']['id'].$product['size'].$product['color'].str_replace(str_split(' ,'),'',$product['values']) }}">
                      <td class="product-img">
                        <div class="card-item">
                          <img width="50" src="{{ $product['item']['photo'] ? asset('assets/images/products/'.$product['item']['photo']):asset('assets/images/noimage.png') }}" alt="">
                          <p class="name"><a href="{{ route('front.product', $product['item']['slug']) }}">@if(isJSON($product['item']['name'])){{ mb_strlen(json_decode($product['item']['name'],true)[$langg->id],'utf-8') > 35 ? mb_substr(json_decode($product['item']['name'],true)[$langg->id],0,35,'utf-8').'...' : json_decode($product['item']['name'],true)[$langg->id] }}@else{{ mb_strlen($product['item']['name'],'utf-8') > 35 ? mb_substr($product['item']['name'],0,35,'utf-8').'...' : $product['item']['name'] }} @endif</a></p>
                        </div>
                      </td>
                                            <td class="sub__total">
