...
<div class="row">
    ...
    @if($prod->user_id != 0)
        ...
        @if($prod->user->is_vendor == 2)
            <div class="col-md-4 col-lg-2 col-6 p-1">
                <a href="{{ route('front.product', $prod->slug) }}" class="item">
                    ...
                    <div class="info">
                        <h5 class="name">@if(isJSON($prod->name)){{json_decode($prod->name,true)[$langg->id]}}@else {{$prod->name}} @endif
                            </h5>
                        <div class="stars">
                            <div class="ratings">
                        ...
                        @endphp
                        @if($percentage>0)
                            <p class="text-danger mb-2">{{$percentage .'% off' }}</p>
                        @endif
                    </div>
                    <div class="item-cart-area">
                            @if($prod->product_type == "affiliate")
                                <span class="add-to-cart-btn affilate-btn"
                                    data-href="{{ route('affiliate.product', $prod->slug) }}"><i class="sagor"></i>
                                                                            {{ $langg->lang251 }}
                                                                        </span>
                            @else
                                @if($prod->emptyStock())
...
<div class="load_more text-center mt-1">
    <button type="button" name="load_more_button" class="btn shadow text-white px-2 load__btn btn-sm rounded-0" data-id="{{ $last_id }}" id="load_more_button"><i class="fas fa-plus-circle"></i> Load more</button>
</div>
@endif
