      ...
      <div class="col-lg-12">
        <ul class="pages">
          ...
          <li><a href="{{route('front.category',$productt->category->slug)}}">
                  @if(isJSON($productt->category->name)){{json_decode($productt->category->name,true)[$langg->id]}}@else {{$productt->category->name}} @endif
              </a></li>
          @if($productt->subcategory_id != null)
          <li><a
              href="{{ route('front.subcat',['slug1' => $productt->category->slug, 'slug2' => $productt->subcategory->slug]) }}">
                  @if(isJSON($productt->subcategory->name)){{json_decode($productt->subcategory->name,true)[$langg->id]}}@else {{$productt->subcategory->name}} @endif
              </a>
          </li>
          @endif
          @if($productt->childcategory_id != null)
          <li><a
              href="{{ route('front.childcat',['slug1' => $productt->category->slug, 'slug2' => $productt->subcategory->slug, 'slug3' => $productt->childcategory->slug]) }}">
                  @if(isJSON($productt->childcategory->name)){{json_decode($productt->childcategory->name,true)[$langg->id]}}@else {{$productt->childcategory->name}} @endif

              </a>
          </li>
          @endif
          <li><a href="{{ route('front.product', $productt->slug) }}">@if(isJSON($productt->name)){{json_decode($productt->name,true)[$langg->id]}}@else {{$productt->name}} @endif</a>
          ...
          <div class="xzoom-container">
            ...
            <div class="col-lg-7">
              <div class="right-area">
                <div class="product-info">
                  <h4 class="product-name">@if(isJSON($productt->name)){{json_decode($productt->name,true)[$langg->id]}}@else {{$productt->name}} @endif</h4>
            ...
            <div class="product-price">
              <p class="title">{{ $langg->lang87 }} :</p>
                @php
                    $price = preg_replace('/[^0-9,.]+/', '', $productt->showPrice());
           $previous_price=preg_replace('/[^0-9,.]+/', '', $productt->showPreviousPrice());
           $percentage=0;
           if ($previous_price!=""){
           ...
           $percentage =number_format( ( $diff/ $previous_price)*100);
                ...
                @endphp
                    <p class="price"><span id="sizeprice">{{ $productt->showPrice() }}</span>
                      <small><del>{{ $productt->showPreviousPrice() }}</del></small> @if($percentage>0)
                  ...
                  <div class="info-meta-3">
                    <ul class="meta-list">
                      ...
                      @else
                      <li class="addtocart">
                        <a href="javascript:;" id="addcrt"><i class="sagor"></i>{{ $langg->lang90 }}</a>
                      </li>

                      <li class="addtocart">
                        <a id="qaddcrt" href="javascript:;">
                          <i class="sagor"></i>{{ $langg->lang251 }}
                        </a>
                      </li>
                      @endif
                      ...
                      <li class="favorite" data-toggle="tooltip" data-placement="top" title="{{ $langg->lang9 }}">
                  ...
                  <div id="product-details-tab">
                    ...
                    <div class="tab-content-wrapper">
                      <div id="tabs-1" class="tab-content-area">
                        <p>@if(isJSON($productt->details)){!!json_decode($productt->details,true)[$langg->id]!!}@else {!! $productt->details !!} @endif</p>
                      </div>
                      <div id="tabs-2" class="tab-content-area">
                        <p>@if(isJSON($productt->policy)){!!json_decode($productt->policy,true)[$langg->id]!!}@else {!! $productt->policy !!} @endif</p>
                      </div>
  ...
  <div class="container-fluid">
    ...
    <div class="row">
      <div class="col-lg-12 px-lg-4">
        <div class="row">
          @foreach($productt->category->products()->where('status','=',1)->where('id','!=',$productt->id)->take(6)->get()
          as $prod)
